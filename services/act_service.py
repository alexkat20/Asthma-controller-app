"""
Тест контроля астмы (по мотивам стандартного клинического опросника ACT —
Asthma Control Test). Официальный ACT — лицензированный инструмент, поэтому
формулировки вопросов ниже переписаны своими словами, а не скопированы; логика
и шкала оценки (5 вопросов по 1-5 баллов, сумма 5-25) сохраняют тот же смысл:
чем ниже балл — тем хуже контролируется астма.

Первый тест предлагается сразу после заполнения профиля (см. profile_service),
дальше — раз в 30 дней после последнего пройденного теста; отсчёт при полном
отсутствии тестов идёт от даты создания профиля (то есть от первого
...
# Каждый вопрос: (текст, [(вариант_ответа, балл), ...]) — от худшего к лучшему.
ACT_QUESTIONS = [
    (
...
def continue_act(user_id: str, session: dict, text: str) -> ChatOut:
    ...
    # Тест пройден — считаем и сохраняем результат.
