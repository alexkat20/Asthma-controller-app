"""
Отчёт для визита к врачу — самодостаточный HTML-файл (графики встроены как
base64, внешних зависимостей нет), который можно открыть в браузере и
распечатать/сохранить как PDF через обычный диалог печати (Ctrl+P).

Почему HTML, а не PDF-библиотека: избегаем проблем с шрифтами для кириллицы
(частая боль генерации PDF из Python) — браузер и так умеет корректно
показывать/печатать русский текст, а печать в PDF — стандартная функция
любого браузера в один клик.
"""
